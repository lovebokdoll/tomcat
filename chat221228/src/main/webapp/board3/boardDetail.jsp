...
<%
List<Map<String, Object>> getBoardList = (List<Map<String, Object>>) request.getAttribute("bList");
...
<script type="text/javascript">
	...
	function boardDelView() {
		...
		$('#d_boardDel').dialog({
			...
			closed : true,
			cache : false,
			href: 'boardDelete.jsp?bm_no=<%=bm_no%>&bm_pw=<%=bm_pw%>',
			modal : true
		});
		$('#d_boardDel').dialog('open');
	}
	//글삭제 화면에서 확인 버튼을 클릭했을때 
	function boardDel() {
		const db_pw ='<%=bm_pw%>'
...
<body>
  ...
  <table align="center" id="p" class="easyui-panel" title="글상세보기" data-options="footer:'#tb_read'"
    ...
    <tr>
      <td>제목</td>
      <td>
        <input id="bm_title" value="<%=bm_title%>" name="bm_title" data-options="width:'450px'" class="easyui-textbox">
      </td>
    </tr>
    <tr>
      <td>작성자</td>
    ...
    <tr>
      <td>내용</td>
      <td>
        <input id="bm_content" value="<%=bm_content%>" name="bm_content" data-options="multiline:'true', width:'570px', height:'90px'" class="easyui-textbox">
      </td>
    </tr>
    <tr>
      <td>비밀번호</td>
      <td>
        <input id="bm_pw" value="<%=bm_pw%>" name="bm_pw" class="easyui-passwordbox">
      </td>
    </tr>
  </table>
